...
//        $this->middleware('auth');
    ...
    public function subscribeApp(Request $request)
    {
        ...
        if ($arrayReq['id']) {
            $app = App::where('id', $arrayReq['id'])->first();
            $platform = Platform::where(['name' => Lib::getPlatformByMsisdn($arrayReq['subscriberId'])])->first();
            if ($app && $platform) {
                $doPlatform = $app->platforms->find($platform->id);
                $diPivot = null;
                if ($doPlatform) {
                    $diPivot = $doPlatform->pivot;
                }
                if ($diPivot) {
                    $dataRequest = [
                        "applicationId" => $diPivot->subscription_app_id,
                        "password" => $diPivot->subscription_password,
                    ...
                    $subResp = [
                        'error' => true,
                        'message' => 'We can\'t find the Application Platform. Please try again later.'
                    ];
                }
            } else {
                $subResp = [
            ...
            if ($app) {
                ...
                if ($app && $platform) {
                    ...
                    if ($diPivot) {
                        ...
                        $subResp = [
                            'error' => true,
                            'message' => 'We can\'t find the Platform. Please try again later.'
                        ];
                    }
                } else {
                    $subResp = [
